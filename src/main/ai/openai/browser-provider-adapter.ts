 ...
 */
const extractUserIdentifier = (identifier: string): Effect.Effect<string, Error, ElectronSessionService> =>
  Effect.gen(function* () {
    ...
    const userIdentifier = yield* Effect.tryPromise({
      try: async () => {
        try {
          ...
          const response = await accountSession.fetch('https://chatgpt.com/backend-api/accounts/check', {
            headers: {
              'User-Agent':
...
export class OpenAiBrowserProviderAdapter extends Effect.Service<OpenAiBrowserProviderAdapter>()(
  ...
  {
    dependencies: [BrowserAuthService.Default, CookieUsagePageAdapter.Default, ElectronSessionService.Default],
      ...
      const adapter: AiProviderAdapter = {
        ...
        signIn: () =>
          Effect.gen(function* () {
            ...
            yield* Console.log(`[OpenAI] Starting sign-in flow with temp identifier: ${tempIdentifier}`)
            ...
            yield* Console.log(`[OpenAI] Auth window closed, checking cookies for temp session...`)

            // Verify cookies exist in temp session
            const tempSession = yield* sessionService.getSession(PROVIDER, authResult.identifier)
            const tempHasCookies = yield* sessionService.hasAnyCookies(tempSession)
            yield* Console.log(`[OpenAI] Temp session has cookies: ${tempHasCookies}`)

            if (!tempHasCookies) {
              yield* Console.error(`[OpenAI] CRITICAL: No cookies found in temp session after auth!`)
            }

            // Extract stable user identifier (email or account ID)
            const stableIdentifier = yield* extractUserIdentifier(authResult.identifier).pipe(
              Effect.provide(ElectronSessionService.Default),
              // If extraction fails, use the temp identifier
              Effect.catchAll((error) =>
                Console.log(`[OpenAI] Failed to extract stable identifier, using temp: ${error}`).pipe(
                  Effect.flatMap(() => Effect.succeed(authResult.identifier))
                )
              )
            )

            yield* Console.log(`[OpenAI] Using stable identifier for account: ${stableIdentifier}`)
            ...
            if (stableIdentifier !== authResult.identifier) {
              ...
              const stableSession = yield* sessionService.getSession(PROVIDER, stableIdentifier)

              // Check stable session before migration
              const stableHasCookiesBefore = yield* sessionService.hasAnyCookies(stableSession)
              yield* Console.log(`[OpenAI] Stable session has cookies before migration: ${stableHasCookiesBefore}`)

              // Copy cookies from temp session to stable session
              yield* sessionService.copyCookies(tempSession, stableSession).pipe(
                Effect.tap(() => Console.log(`[OpenAI] Successfully copied cookies to stable session`)),
                Effect.catchAll(error => {
                  // Log error but don't fail - cookies might already be there
                  return Console.error(`[OpenAI] Failed to copy cookies: ${error}`)
                })
              )

              // Verify cookies were copied
              const stableHasCookiesAfter = yield* sessionService.hasAnyCookies(stableSession)
              yield* Console.log(`[OpenAI] Stable session has cookies after migration: ${stableHasCookiesAfter}`)

              if (!stableHasCookiesAfter) {
                yield* Console.error(`[OpenAI] CRITICAL: Cookie migration failed! Stable session has no cookies`)
              }

              // Clean up temp session
              yield* sessionService.clearSessionData(tempSession).pipe(
                Effect.tap(() => Console.log(`[OpenAI] Cleaned up temp session`)),
                Effect.catchAll(error => {
                  return Console.error(`[OpenAI] Failed to clear temp session: ${error}`)
                })
              )
            } else {
              yield* Console.log(`[OpenAI] Using temp session as stable session (no migration needed)`)
            }

            const accountId = AiAccount.makeAccountId(PROVIDER, stableIdentifier)
            yield* Console.log(`[OpenAI] Created account ID: ${accountId}`)

            // Verify final session has cookies
            const finalSession = yield* sessionService.getSession(PROVIDER, stableIdentifier)
            const finalHasCookies = yield* sessionService.hasAnyCookies(finalSession)
            yield* Console.log(`[OpenAI] Final session (${stableIdentifier}) has cookies: ${finalHasCookies}`)

            if (!finalHasCookies) {
              yield* Console.error(`[OpenAI] CRITICAL: Final session has no cookies before returning!`)
            }
        ...
        checkAuth: accountId =>
          Effect.gen(function* () {
            yield* Console.log(`[OpenAI] Checking auth for account: ${accountId}`)
            ...
            yield* Console.log(`[OpenAI] Account ${accountId} authenticated: ${authenticated}`)
