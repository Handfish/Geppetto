...
type ContractInput<K extends keyof typeof AiProviderIpcContracts> = S.Schema.Type<
  (typeof AiProviderIpcContracts)[K]['input']
>
type ContractOutput<K extends keyof typeof AiProviderIpcContracts> = S.Schema.Type<
  (typeof AiProviderIpcContracts)[K]['output']
>
  ...
  const setupHandler = <K extends keyof typeof AiProviderIpcContracts, E>(
    ...
    ipcMain.handle(contract.channel, async (_event, input: unknown) => {
      const program = Effect.gen(function* () {
        const validatedInput = yield* S.decodeUnknown(contract.input as unknown as InputSchema)(
          input
        )
