# Watcher Activity Detection: Node-PTY and Tmux Control Mode

## Problem Statement

When users spawn AI watchers that run in tmux sessions accessed via external terminal emulators (like Ghostty), the watcher UI icons need to reflect real-time activity - turning green when the user types in that session.
...
Activity detector marks process as active
    ↓
Watcher icon turns green
```

### Implementation Details

**TmuxControlClient** (`src/main/ai-watchers/adapters/tmux-control-client-adapter.ts`):
...
**Integration** (`src/main/ai-watchers/adapters/tmux-session-manager-adapter.ts`):
- When creating a watcher session, extracts the pane ID from tmux
- Calls `pipeTmuxSession()` to start control mode monitoring
- One control mode client per watcher session
...
4. Watcher icons reflect real-time state

## Known Limitations

### Performance: Window Switching

When multiple control mode clients are active (one per watcher), `tmux switch-client` becomes slow due to resource contention. This is a tmux limitation, not a defect in our implementation.

**Why it happens:**
- Each watcher maintains an attached control mode PTY client
...
**Trade-off accepted:**
- **Gained**: Real-time activity detection for all watchers
...
5. **Reduce control mode clients**: Would lose activity detection for watchers we're not actively monitoring
...
4. **Reduce monitoring scope**: Only monitor foreground watcher, not all watchers

## Code References

- **Control mode client**: `src/main/ai-watchers/adapters/tmux-control-client-adapter.ts`
- **Session manager**: `src/main/ai-watchers/adapters/tmux-session-manager-adapter.ts`
- **Process monitor integration**: `src/main/ai-watchers/adapters/node-process-monitor-adapter.ts` (lines ~830-900)
...
Enable detailed logging by looking for `[TmuxControl:...]` log messages in the dev console when watchers are active.
